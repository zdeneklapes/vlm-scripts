#!/usr/bin/env python3
"""Run agentic inference with Qwen/Qwen3-VL-4B-Thinking.

This model emits thinking and tool calls alongside its answer
(<think>...</think> + <tool_call>{json}</tool_call>). Output is parsed into separate
...
generation settings. The model is downloaded automatically on first run:
    ...
    export MODEL_URL_HF_QWEN3_VL_4B_THINKING='https://...'
    uv run models/Qwen3-VL-4B-Thinking/run.py
...
# --- Model -------------------------------------------------------------------
MODEL_ID = "Qwen/Qwen3-VL-4B-Thinking"  # HuggingFace ID -- used as base model for LoRA adapters
MODEL_URL_ENV = "MODEL_URL_HF_QWEN3_VL_4B_THINKING"  # env var holding the model download URL
FAMILY = "qwen3vl"  # wire-format family: qwen3vl | lfm2.5 | gemma4
...
MAX_TOKENS = 3072  # thinking + tool calls need more room than a plain answer
